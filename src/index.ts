document.addEventListener("DOMContentLoaded", () => {
  ...
  api.moviesListData().then((data) => {
    if (data) content.movieListContent(data);
  });

  api.moviesListFront().then((item) => {
    if (item) content.moviesFrontContent(item);
  });

  api.moviesTrending().then((param) => {
    if (param) content.trendingContent(param);
  });

  api.moviesTopRated().then((element) => {
    if (element) content.topRatingContent(element);
  });

  api.moviesUpComing().then((veri) => {
    if (veri) content.upComingContent(veri);
  });
